...
def run_bot(username, active_key, profit_target=1.0, scalping_enabled=False):
    ...
    if rc_percent is not None:
        ...
        print(f"[LTC BOT] Resource Credits: Unable to fetch.")
    ...
    if not market:
        ...
        if pek_ask <= 0:
            pek_ask = 0.00000001
            print(f"[LTC BOT] PEK market ask unavailable, using fallback price {pek_ask}")
        try:
            place_order(username, "PEK", pek_ask, 0.00000001, order_type="buy", active_key=active_key, nodes=HIVE_NODES)
            print(f"[LTC BOT] Bought 0.00000001 PEK at {pek_ask}")
        except Exception as e:
            print(f"[LTC BOT] PEK buy exception: {e}")
        time.sleep(2)
        return
    ...
    print(f"[LTC BOT] Preparing BUY: {buy_qty} {TOKEN} at {buy_price}")
    ...
    try:
        ...
        print(f"[LTC BOT] Bought 0.00000002 PEK at {pek_ask}")
    except Exception as e:
        print(f"[LTC BOT] PEK buy exception: {e}")
    time.sleep(2)
    ...
    try:
        ...
        print(f"[LTC BOT] Bought 0.00000001 {TOKEN} at {ask}")
    except Exception as e:
        print(f"[LTC BOT] {TOKEN} self-buy exception: {e}")
    time.sleep(2)
    print(f"[LTC BOT] Preparing SELL: {sell_qty} {TOKEN} at {sell_price}")
    ...
    if buy_qty <= 0:
        print(f"[LTC BOT] Skipping BUY: buy_qty is zero or negative. Check HIVE balance and buy price.")
    elif duplicate_buy:
        print(f"[LTC BOT] Skipping BUY: Duplicate buy order at {buy_price} detected.")
    else:
        try:
            place_order(username, TOKEN, buy_price, buy_qty, order_type="buy", active_key=active_key, nodes=HIVE_NODES)
            print(f"[LTC BOT] BUY order submitted: {buy_qty} {TOKEN} at {buy_price}")
            time.sleep(5)
            open_orders = get_open_orders(username, TOKEN)
            if open_orders:
                print(f"[LTC BOT] Open orders after BUY: {len(open_orders)} found.")
            else:
                print(f"[LTC BOT] No open orders found after BUY (may be node delay).")
            time.sleep(1)
        except Exception as e:
    ...
    if force_sell_price > buy_price and sell_qty > 0:
        ...
        if duplicate_sell:
            print(f"[LTC BOT] Skipping SELL: Duplicate sell order at {force_sell_price} detected.")
        else:
            try:
                place_order(username, TOKEN, force_sell_price, sell_qty, order_type="sell", active_key=active_key, nodes=HIVE_NODES)
                print(f"[LTC BOT] SELL order submitted: {sell_qty} {TOKEN} at {force_sell_price}")
                print(f"[LTC BOT] Profit percent: {get_profit_percent(buy_price, force_sell_price)}%")
                time.sleep(5)
                open_orders = get_open_orders(username, TOKEN)
                if open_orders:
                    print(f"[LTC BOT] Open orders after SELL: {len(open_orders)} found.")
                else:
                    print(f"[LTC BOT] No open orders found after SELL (may be node delay).")
                time.sleep(1)
            except Exception as e:
                print(f"[LTC BOT] SELL order exception: {e}")
    else:
