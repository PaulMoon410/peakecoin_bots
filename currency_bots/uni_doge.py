...
def run_bot(username, active_key, profit_target=1.0):
    ...
    try:
        ...
        print(f"[DOGE BOT] PEK buy exception: {e}")
    time.sleep(2)
    print(f"[DOGE BOT] Preparing SELL: {sell_qty} {TOKEN} at {sell_price}")
    ...
    if force_sell_price > buy_price and sell_qty > 0:
        ...
        if duplicate_sell:
            print(f"[DOGE BOT] Skipping SELL: Duplicate sell order at {force_sell_price} detected.")
        else:
            try:
                ...
                print(f"[DOGE BOT] Profit percent: {get_profit_percent(buy_price, force_sell_price)}%")
                time.sleep(5)
                open_orders = get_open_orders(username, TOKEN)
                if open_orders:
        ...
        print(f"[DOGE BOT] SELL order skipped: Not profitable or sell_qty is zero.")
    print(f"[DOGE BOT] Trade cycle for {TOKEN} complete.")
    print("==============================\n")
    time.sleep(DELAY)
